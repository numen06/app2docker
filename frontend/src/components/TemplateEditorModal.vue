    ...
    <div class="mb-4 grid gap-4 sm:grid-cols-2">
      <div class="space-y-2">
        ...
        <p v-if="isBuiltin" class="text-xs text-amber-700">
          <AppIcon  name="info-circle" /> 内置模板不可重命名，保存后将在用户目录创建覆盖
        </p>
      </div>
      <div class="space-y-2">
        ...
        <p v-if="!canChangeProjectType" class="text-xs text-amber-700">
          <AppIcon  name="lock" /> 内置模板的项目类型不可修改
        </p>
        <p v-else-if="!isNew && projectTypeChanged" class="text-xs text-sky-700">
          <AppIcon  name="info-circle" /> 修改项目类型后，模板将移动到新目录
        </p>
        <p v-else class="text-xs text-slate-500">
          <AppIcon  name="lightbulb" /> 请从下拉列表中选择项目类型
        </p>
      </div>
    </div>
