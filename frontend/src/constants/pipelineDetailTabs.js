...
export const PIPELINE_CONFIG_TAB_KEYS = ["basic","git","services","build","dockerfile","resource","webhook","post_webhook","other",
];
...
const TAB_META = {
  basic: { label:"基本信息", icon:"info-circle" },
  git: { label:"Git 配置", icon:"code-branch" },
  services: { label:"多服务配置", icon:"layer-group" },
  build: { label:"构建 JSON", icon:"code" },
  dockerfile: { label:"Dockerfile", icon:"file-code" },
  resource: { label:"资源包", icon:"archive" },
  webhook: { label:"Webhook", icon:"link" },
  post_webhook: { label:"构建后 Webhook", icon:"bell" },
  other: { label:"其他选项", icon:"sliders-h" },
};
