...
class SSHExecutor(DeployExecutor):
    ...
    async def execute(
        ...
        Returns:
            执行结果字典
        """
        logger.info(f"[SSH] 开始执行 SSH 部署任务: task_id={task_id}, target={target_name}, host={self.host_name}")
        ...
        try:
            ...
            logger.info(f"[SSH] 主机配置: {self.host_name} ({host_config['host']}:{host_config['port']})")
            ...
            if steps and isinstance(steps, list):
                # 多步骤模式：依次执行每个步骤
                if update_status_callback:
                    update_status_callback(f"[SSH] 开始执行多步骤部署，共 {len(steps)} 个步骤...")
        ...
        try:
            ...
            try:
                ...
                for idx, step in enumerate(steps, 1):
                    ...
                    if update_status_callback:
                        update_status_callback(f"[步骤 {idx}/{len(steps)}] {step_name}: {step_command}")
                    
                    logger.info(f"[SSH] 执行步骤 {idx}/{len(steps)}: {step_name} - {step_command}")
                    ...
                    if exit_status != 0:
                        ...
                        if "kill" in step_command.lower() or "stop" in step_command.lower() or "rm" in step_command.lower():
                            logger.info(f"[SSH] 步骤 {idx} 失败但可接受（清理操作）")
                        # 检查是否是容器名称冲突错误
                        elif "container name" in error_text and "already in use" in error_text:
                            logger.warning(f"[SSH] 步骤 {idx} 检测到容器名称冲突，尝试自动清理...")
                            
                            # 尝试从错误信息中提取容器名称
                            import re
                            container_name_match = re.search(r'container name[^"]*"([^"]+)"', error_text)
                            if not container_name_match:
                                # 尝试从命令中提取容器名称
                                name_match = re.search(r'--name[=\s]+([^\s]+)', step_command)
                                if name_match:
                            ...
                            if container_name:
                                ...
                                try:
                                    stdin_cleanup, stdout_cleanup, stderr_cleanup = ssh_client.exec_command(cleanup_cmd)
                                    cleanup_exit = stdout_cleanup.channel.recv_exit_status()
                                    cleanup_output = stdout_cleanup.read().decode("utf-8", errors='ignore')
                                    logger.info(f"[SSH] 清理命令执行结果: exit_status={cleanup_exit}, output={cleanup_output}")
                                    
                                    if update_status_callback:
                                        update_status_callback(f"[自动清理] 已删除冲突容器 {container_name}，重新执行步骤 {idx}...")
                                    
                                    # 重新执行失败的步骤
                                    logger.info(f"[SSH] 重新执行步骤 {idx}: {step_command}")
                                    stdin_retry, stdout_retry, stderr_retry = ssh_client.exec_command(step_command)
                                    retry_exit_status = stdout_retry.channel.recv_exit_status()
                                    retry_stdout_text = stdout_retry.read().decode("utf-8", errors='ignore')
                                    retry_stderr_text = stderr_retry.read().decode("utf-8", errors='ignore')
                                    ...
                                    if retry_exit_status != 0:
                                        logger.error(f"[SSH] 步骤 {idx} 重试后仍然失败: exit_status={retry_exit_status}")
                            ...
                            # 其他类型的错误，停止执行
                            logger.error(f"[SSH] 步骤 {idx} ({step_name}) 执行失败: exit_status={exit_status}")
                            failed_step = step_output
                            break
                ...
                # 构建结果
                if failed_step:
                    error_msg = f"步骤 {failed_step['step']} ({failed_step['name']}) 执行失败"
                    ...
                    return {
                        "success": False,
                        "message": error_msg,
                        "output": "\n".join([f"[步骤 {o['step']}] {o['name']}\n{o['stdout']}" for o in all_outputs]),
