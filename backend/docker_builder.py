...
class DockerBuilder(ABC):
    ...
    @abstractmethod
    def push_image(self, repository: str, tag: str = "latest", auth_config: Optional[Dict] = None) -> Iterator[Dict]:
        """
        推送镜像到仓库
        Args:
            repository: 仓库名称
    ...
    @abstractmethod
    def pull_image(self, repository: str, tag: str = "latest", auth_config: Optional[Dict] = None) -> Iterator[Dict]:
        """
        拉取镜像
        Args:
            repository: 仓库名称
    ...
    def build_image(self, path: str, tag: str, **kwargs) -> Iterator[Dict]:
        ...
        if hasattr(self, 'auth_config') and self.auth_config:
            try:
                ...
                print(f"✅ 已登录到仓库: {self.auth_config.get('serveraddress', 'docker.io')}")
            except Exception as e:
                print(f"⚠️ 仓库登录失败: {e}")
